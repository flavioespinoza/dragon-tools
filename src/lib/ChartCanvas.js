...
class ChartCanvas extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (this.panInProgress) {
			...
			if (!reset) {
				state.chartConfig
					.forEach((each) => {
						const prevChartConfig = initialChartConfig.filter(d => d.id === each.id);
						if (prevChartConfig) {
							each.yScale.domain(prevChartConfig[0].yScale.domain());
							each.yPanEnabled = prevChartConfig[0].yPanEnabled;
						}
					});
			}
    ...
    // console.log('prevState', prevState);
		// console.log(this.state.chartConfig, this.state.chartConfig.map(d => d.yScale.domain()));
	}
